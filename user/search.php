...
if (!empty($params)) {
    // Fix binding parameters for count query
    $bind_types = substr($types, 0, strlen($types)); // Get only the types for existing params
...
if (!empty($params)) {
    // Fix binding parameters by using the spread operator for all params
...
<div class="row">
    <div class="col-md-12">
        <h2>Search Books</h2>
        <p>Browse our library collection and borrow books of your interest.</p>
    </div>
</div>
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card h-100">
                            <div class="card-body">
                                ...
                                <div class="mt-auto">
                                    <a href="book-details.php?id=<?= $book['id'] ?>" class="btn btn-info btn-sm">Details</a>
                                    <?php if ($book['available'] > 0 && canUserBorrowMore($_SESSION['user_id'])): ?>
                                        <a href="borrow.php?id=<?= $book['id'] ?>" class="btn btn-primary btn-sm">Borrow</a>
                                    <?php elseif (!canUserBorrowMore($_SESSION['user_id'])): ?>
        ...
        <?php else: ?>
            <div class="alert alert-info">
                ...
                <?php if (!empty($search) || $category > 0): ?>
                    <a href="search.php" class="btn btn-outline-primary">Clear Search</a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </div>
